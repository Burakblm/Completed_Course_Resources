...
data = pd.read_csv("gt.txt",names = col_list) # names parametresi stünlara isim dizisi vermeyi sağlar

car = data[data["class"] == 3] # car class
...
df = pd.DataFrame(bound_box_list, columns=["frame_no", "x", "y", "genis", "yuksek", "center_x", "center_y"])
df.to_csv("gt_new.txt",index=False)# data frameyi dosya şekline kaydetmeye yarar
