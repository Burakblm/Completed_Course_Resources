...
print("Label(sınıf) sayisi: ",noOfClasses)
...
# sns.countplot(y_validation,ax = axes[0])
...
model = Sequential()
model.add(Conv2D(input_shape=(28,28,1),filters=8,kernel_size=(5,5),activation="relu",padding="same"))
...
open("model_trained_new_mnist.json","w").write(model.to_json())# modeli kaydetmek için kullanılır
model.save_weights("model_training_mnist.h5")
...
score = model.evaluate(x_test,y_test,verbose = 1)# verbose yüklenme cubugunu göstermeye yarar
